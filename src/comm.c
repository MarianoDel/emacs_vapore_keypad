...
unsigned char InterpretarMsg (unsigned char lstate, char * pStr)	//TODO copiar payload solo en las cuestiones de LCD y no en los comandos
{
	char str [64];

	if (strncmp(pStr, (const char *) "Starting Goto 115200\r", sizeof((const char *) "Starting Goto 115200\r") - 1) == 0)
	{
		return MAIN_TO_MONITORING;
	}

	if (strncmp(pStr, (const char *) "Leaving Goto 9600\r", sizeof((const char *) "Leaving Goto 9600\r") - 1) == 0)
	{
