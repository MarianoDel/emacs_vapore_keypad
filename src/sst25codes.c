...
void MemoryDump (unsigned int address)
{
	unsigned char i;
	char str [64];
...
//grabar codigo en sst
unsigned char WriteCodeToMemory_SST(unsigned short position, unsigned int new_code)
{
...
unsigned short CheckBaseMask_SST (unsigned int code_to_find, unsigned int mask)
{
	unsigned short i;
	unsigned int * p_mem;
	//char str [64];
...
unsigned char CheckButtonInCode_SST (unsigned int code_to_check, unsigned int button_mask)
{
...
unsigned int CheckIndexInMemory_SST (unsigned short index_to_find)
{
	unsigned int * p_mem;
